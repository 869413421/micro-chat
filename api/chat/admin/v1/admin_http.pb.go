...
func RegisterChatAdminHTTPServer(s *http.Server, srv ChatAdminHTTPServer) {
	...
	r.POST("/admin/v1/user/{userId}/role", _ChatAdmin_SetUserRole0_HTTP_Handler(srv))
	r.DELETE("/admin/v1/user/{userId}/role", _ChatAdmin_DeleteUserRole0_HTTP_Handler(srv))
}
...
func _ChatAdmin_SetUserRole0_HTTP_Handler(srv ChatAdminHTTPServer) func(ctx http.Context) error {
	return func(ctx http.Context) error {
		var in SetUserRoleRequest
		if err := ctx.Bind(&in); err != nil {
			return err
		}
		if err := ctx.BindVars(&in); err != nil {
		...
		http.SetOperation(ctx, OperationChatAdminSetUserRole)
		h := ctx.Middleware(func(ctx context.Context, req interface{}) (interface{}, error) {
			return srv.SetUserRole(ctx, req.(*SetUserRoleRequest))
		})
		out, err := h(ctx, &in)
		if err != nil {
			return err
		}
		reply := out.(*SetUserRoleResponse)
		return ctx.Result(200, reply)
	}
}

func _ChatAdmin_DeleteUserRole0_HTTP_Handler(srv ChatAdminHTTPServer) func(ctx http.Context) error {
	return func(ctx http.Context) error {
		...
		http.SetOperation(ctx, OperationChatAdminDeleteUserRole)
		h := ctx.Middleware(func(ctx context.Context, req interface{}) (interface{}, error) {
			return srv.DeleteUserRole(ctx, req.(*DeleteUserRoleRequest))
		})
		out, err := h(ctx, &in)
		if err != nil {
			return err
		}
		reply := out.(*DeleteUserRoleResponse)
		return ctx.Result(200, reply)
	}
}
...
func (c *ChatAdminHTTPClientImpl) DeleteUserRole(ctx context.Context, in *DeleteUserRoleRequest, opts ...http.CallOption) (*DeleteUserRoleResponse, error) {
	var out DeleteUserRoleResponse
	pattern := "/admin/v1/user/{userId}/role"
	path := binding.EncodeURL(pattern, in, true)
	opts = append(opts, http.Operation(OperationChatAdminDeleteUserRole))
	opts = append(opts, http.PathTemplate(pattern))
	err := c.cc.Invoke(ctx, "DELETE", path, nil, &out, opts...)
	if err != nil {
		return nil, err
...
func (c *ChatAdminHTTPClientImpl) SetUserRole(ctx context.Context, in *SetUserRoleRequest, opts ...http.CallOption) (*SetUserRoleResponse, error) {
	var out SetUserRoleResponse
	pattern := "/admin/v1/user/{userId}/role"
	path := binding.EncodeURL(pattern, in, false)
	opts = append(opts, http.Operation(OperationChatAdminSetUserRole))
	opts = append(opts, http.PathTemplate(pattern))
	err := c.cc.Invoke(ctx, "POST", path, in, &out, opts...)
	if err != nil {
		return nil, err
